...
public class CraftChunk implements Chunk {
    ...
    private final int x;
    private final int z;
    private static final DataPaletteBlock<IBlockData> emptyBlockIDs = new DataPaletteBlock<>(Blocks.AIR.defaultBlockState(), Strategy.createForBlockStates(net.minecraft.world.level.block.Block.BLOCK_STATE_REGISTRY));
    ...
    public ChunkSnapshot getChunkSnapshot(boolean includeMaxBlockY, boolean includeBiome, boolean includeBiomeTempRain) {
        ...
        Codec<PalettedContainerRO<Holder<BiomeBase>>> biomeCodec = palettedcontainerfactory.biomeContainerCodec();

        for (int i = 0; i < cs.length; i++) {
            NBTTagCompound data = new NBTTagCompound();

            data.put("block_states", palettedcontainerfactory.blockStatesContainerCodec().encodeStart(DynamicOpsNBT.INSTANCE, cs[i].getStates()).getOrThrow());
            sectionBlockIDs[i] = palettedcontainerfactory.blockStatesContainerCodec().parse(DynamicOpsNBT.INSTANCE, data.getCompoundOrEmpty("block_states")).getOrThrow(SerializableChunkData.a::new);
        ...
        return new CraftChunkSnapshot(getX(), getZ(), chunk.getMinY(), chunk.getMaxY(), world.getSeaLevel(), world.getName(), world.getFullTime(), sectionBlockIDs, sectionSkyLights, sectionEmitLights, sectionEmpty, hmap, biome);
    }
    ...
    public static ChunkSnapshot getEmptyChunkSnapshot(int x, int z, CraftWorld world, boolean includeBiome, boolean includeBiomeTempRain) {
        ...
        Codec<PalettedContainerRO<Holder<BiomeBase>>> biomeCodec = world.getHandle().palettedContainerFactory().biomeContainerCodec();
        ...
        return new CraftChunkSnapshot(x, z, world.getMinHeight(), world.getMaxHeight(), world.getSeaLevel(), world.getName(), world.getFullTime(), blockIDs, skyLight, emitLight, empty, new HeightMap(actual, HeightMap.Type.MOTION_BLOCKING), biome);
    }
