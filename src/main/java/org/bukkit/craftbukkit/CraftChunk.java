...
public class CraftChunk implements Chunk {
    ...
    private final int x;
    private final int z;
    private static final PalettedContainer<net.minecraft.world.level.block.state.BlockState> emptyBlockIDs = new PalettedContainer<>(Blocks.AIR.defaultBlockState(), Strategy.createForBlockStates(net.minecraft.world.level.block.Block.BLOCK_STATE_REGISTRY));
    private static final byte[] FULL_LIGHT = new byte[2048];
    private static final byte[] EMPTY_LIGHT = new byte[2048];

    public CraftChunk(net.minecraft.world.level.chunk.LevelChunk levelchunk) {
        worldServer = levelchunk.q;
    ...
    public ChunkSnapshot getChunkSnapshot(boolean includeMaxBlockY, boolean includeBiome, boolean includeBiomeTempRain) {
        ...
        Codec<PalettedContainerRO<Holder<net.minecraft.world.level.biome.Biome>>> biomeCodec = palettedcontainerfactory.biomeContainerCodec();

        for (int i = 0; i < cs.length; i++) {
            CompoundTag data = new CompoundTag();

            data.put("block_states", palettedcontainerfactory.blockStatesContainerCodec().encodeStart(NbtOps.INSTANCE, cs[i].getStates()).getOrThrow());
            sectionBlockIDs[i] = palettedcontainerfactory.blockStatesContainerCodec().parse(NbtOps.INSTANCE, data.getCompoundOrEmpty("block_states")).getOrThrow(SerializableChunkData.ChunkReadException::new);
